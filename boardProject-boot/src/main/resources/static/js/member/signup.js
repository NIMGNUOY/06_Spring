...
memberEmail.addEventListener("input", e => {
  ...
  if( !regExp.test(inputEmail) ) {
    ...
    emailMessage.classList.remove('confirm');  // 초록색 글자색 제거
    checkObj.memberEmail = false;   // 유효하지 않은 이메일임을 기록
    return;

  }

  // 5) 유효한 이메일 형식인 경우 중복검사 수행
  // 비동기(ajax)
  fetch("/member/checkEmail?memberEmail=" + inputEmail)
  .then( resp => resp.text() )
  .then( count => {
    // count : 1 이면 중복, 0 이면 중복 X
    if(count == 1) {    // 중복 O
...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  if( !checkObj.memberEmail ) {   // 유효한 이메일이 아닌 경우
  ...
  // 메일은 비동기로 서버에서 보내라고 하고 화면에서 타이머 시작하기
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  .then(result => {
    ...
    if(result == 0) { // result 는 string 으로 넘어오기 때문에
...
memberTel.addEventListener("input", e => {
  ...
  telMessage.innerText = "전화번호 입력 완료";
...
signUpForm.addEventListener("submit", e => {
  ...
  // checkObj 의 저장된 값(value) 중 하나라도 false 가 있으면 제출 X
