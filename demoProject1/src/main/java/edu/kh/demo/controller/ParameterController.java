...
@Slf4j	// log 를 이용한 메세지 출력 시 사용 (Lombok 제공)
public class ParameterController {
	 ...
	 * 
	 * required : 입력된 name 속성값의 파라미터 필수 여부 지정(기본값 : true)
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							@RequestParam(value="publisher", required=false, defaultValue="ABC출판사") String publisher) {	// 자동 형변환/파싱
		
		log.debug("title : " + title);
		log.debug("writer : " + writer);
		log.debug("price : " + price);
		log.debug("publisher : " + publisher);		// null
													// defaultValue 값
													// input 입력값
	 ...
	 * 
	 * - 전달 받은 파라미터의 name 속성 값이 같이 사용되는
	 *  DTO의 필드명과 같으면 자동으로 setter 를 호출해서 필드에 값을 세팅
	 * 
	 */
	
	
	// *** @ModelAttribute 를 이용해서 값이 필드에 세팅된 객체를
	//	 "커맨드 객체" 라고 부른다 ***
	
	// *** @ModelAttribute 사용 시 주의 사항 ***
	// - DTO 기본생성자, setter 가 필수로 존재해야 한다!
