...
*
* 설정 방법
* 1) XXXAPPlication.java 파일에 @EnableScheduling 어노테이션 추가(BoardProjectBootApplication)
...
//	@Scheduled(cron = "0, 15, 30, 45, * * * * *")	// 시계 초단위가 0, 15, 30, 45 경우 수행
//	@Scheduled(cron = "0 0 * * * *")	// 정시마다 수행
//	@Scheduled(cron = "0 0 0 * * *")	// 자정마다 수행
//	@Scheduled(cron = "0 0 12 * * *")	// 정오마다 수행
//	@Scheduled(cron = "0,30 * * * * *")	// 시계 초단위가 0, 30 초 일 때 수행(테스트용)
	...
	public void scheduling() {
		...
		// 두 배열을 하나로 합침(for 문 한번만 사용하기 위해서)
		...
		// 배열 내용 복사 (깊은 복사)
		// System.arraycopy
		// (복사할 배열, 몇번 인덱스부터 복사할지, 새로운 배열, 
		// 새로운 배열의 몇번부터 넣을지 인덱스 번호, 복사를 어디까지 할건지)
