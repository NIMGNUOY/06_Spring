...
// await : promise 를 리턴하는 비동기 요청 앞에 붙여 사용하는 키워드 (응답이 올 때까지 기다리겠다)
...
async function getServiceKey() {
    ...
    try {
        ...
        const response = await fetch("/getServiceKey"); // 첫번째 then 의 reasponse(Promise) 객체 리턴
...
async function fetchData() {
    ...
    // await 를 붙이지 않으면 Promise 객체가 출력된다.
...
// var queryParams = '?' + encodeURIComponent('serviceKey') + '='+ getServiceKey(); /*Service Key*/
